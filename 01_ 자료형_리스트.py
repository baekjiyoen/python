...
# 리스트에 담겨있는 음식목록을 출력 해봅시다 - for문
for i in range(10) :
    food = input(f"{i}번째 음식을 입력하세요 :")
    foods.append(food)
...
# 중복된 경우 입력되지 않도록 처리 - while-braek:
f_list = []
while True :
    f = input('좋아하는 과일 ? ')
    ...
    # len(문자열, 리스트, 튜플, 딕셔너리) -> 요소의 갯수
    if len(f_list) == 5 :
        break
    
    print(f_list) # 리스트를 출력하면 리스트에 담긴 요소를 확인 할 수 있다
