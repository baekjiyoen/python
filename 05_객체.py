...
# 인스턴스마다 각각 다른 정보를 저장할 수 있다
print(myClass is myClass2)

class MyClass :
    # 생성자 : 객체가 생성될때 자동으로 호출 되며, 보통 초기화를 진행한다
    # 생성자 이름 :  __init__ , 첫번째 매개변수로 self를 받아 온다
    # self매개변수 : 생성된 객체를 참조(java - this랑 비슷 )
    #             : 인스턴스 변수, 메서드에 접근할 수 있다
    def __init__(self) : 
...
class MyClass :
    ...
    var = '파이썬 클래스'
    # -> None : 반환타입을 명시 (java - void랑 비슷)
    # None : 반환없음
    # 타입힌트는 필수는 아니지만 코드의 가독성을 높이고 오류를 발견하는데 도움을 줄 수 있다
    def __init__(self, name) -> None:
        self.instance_var = '인스턴스 변수'
        self.name = name
      
    # 클래스 자체를 수정 하거나 클래스에 관련된 정보를 반환
    # classmethod : 첫번째 매개변수의 값으로 클래스로 넣어줌
    # @ : 데코레이터
    @classmethod    
    def set_class_var(cls,value) :
        cls.var = value
    # staticmethod -> 생성하지 않고 사용가능    
    @staticmethod
    def static_method() :
        return '정적메서드'
...
# 사람 클래스에 "응애응애"를 화면에 출력하는 cry() 메서드를 추가하라.
class 사람 :
    # 메서드의 첫번째 매개변수로 self를 넣는다
    # 만약 self를 넣지 않으면 TypeError가 발생
    # TypeError : 사람.cry() takes 0 positional arguments but i was given
    def cry(self,소리) :
        # print('응애응애')
        # print(소리*2)
        for 변수 in 소리 :
...
class 사람 : 
    ...
    def __str__(self) -> str:
        ...
        # __str__ : 문자열을 반환하는 메서드
        # 출력할때 실행되는 메서드
        return (f'이름 : {self.name}, 년도 : {self.year}')
        
사람1 = 사람('박서현',2011)
print(사람1) #__str__함수 실행 결과가 반환 되어 져서 출력
...
# 친구를 추가 할때마다 친구들목록에 친구가 추가되어야 한다
class 사람 :
