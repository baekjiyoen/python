...
# len() 함수 매개값을 숫자 타입을 넣으면 오류 발생 '' or ""로 감싸서 문자처리 해줘야함
...
# 오류발생
# print(input_str1.index('오')) # 찾는 문자가 없다면 오류발생
...
print('주민등록번호를 13자리 숫자로 입력')
ssn = input('주민등록번호 : ')
if ssn.isnumeric : 
    if len(ssn) == 13 :
        # 추출 로직
        # 1111112222222
        # 주민등록 번호 뒷자리의 첫번째 자기가 1,2,3,4 인지 판단
        ...
        if int(ssn[6]) in [1,2,3,4] :
            ...
            # 시작번호는 포함, 끝번호는 불포함
            # 시작값(0으로 지정), 종료값(문자열의 길이에 따라 다름)은 생략이 가능
            # :(콜론) 무조건 사용 없을시 한개만 추출
                yy = '19' + yy
                # print(ssn[:2]) # 처음부터 2-1 인덱스 까지 (인덱스는 0부터 시작)
        ...
        print(f'13자리({ssn})로 입력 해주세요')
            # pass : 코드블럭이 없는 경우 입력
else : print(f'숫자({ssn})만 입력 가능합니다')
...
# 내pc 마우스 우클릭 설정 -> 고급시스템 설정 -> 환경변수
# paht에 파이썬설치 경로를 추가(C:/Users/human/AppData/Local/Programs/Python/Python312)
# but 파이썬 설치시 환경변수 추가를 클릭하면 자동으로 환경변수가 등록됨
# 환경변수가 등록되어있는지 확인하는 방법
# 터미널에서 python명령어를 실행
# 등록된 경우 python이 실행되고 명령어를 입력할 수 있는 프롬포트가 출력됨
...
# 자판기에 물품을 초기화
items = ['오징어집','포키','포테이토칩','초코파이']
prices = [2100,1300,1800,1200]

# 인덱스값을 꺼내오기 위해 enumerate함수 이용
