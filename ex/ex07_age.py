...
# 유효성 검증  1. 13자리 2. 숫자변환가능 3. 7번째 자리가 1,2,3,4 중 하나인지
# 유효성 검증 실패시 더이상 코드를 실행하지 않고 
# 다음 반복문으로 넘어감
    if len(ssn) != 13 :
    ...
    if ssn[6] not in ['1','2','3','4'] :
        print('주민등록번호 뒷 1자리는 1,2,3,4 이어야 합니다')
        
    print('주민번호로 성별과 나이를 구하고 콘솔창에 출력')
    if ssn[6] in ['1','3'] :
        print('남자')
    if ssn[6] in ['2','4'] :
        print('여자')
    ...
    if ssn[6] in ['1','2'] :
        ...
        # 숫자와 문자의 연산은 오류가 발생하므로 타입을 맞춰주어야 한다
        if ssn[6] in ['3','4'] :
            yy = '20' + yy
    
    print('나이 : ', 2024 - int(yy))
    res = input('종료하시겠습니까?(Y)')
    # 대문자로 변경 후 비교
    if res.upper() == 'Y' :
